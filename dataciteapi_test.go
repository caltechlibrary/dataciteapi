...
func TestClientDois(t *testing.T) {
	api, err := NewDataCiteClient("datacite_test.go", MailTo)
	if err != nil {
		t.Error(err)
		t.FailNow()
	...
	// test low level getJSON, 10.22002/D1.868 is
	// a Caltech Library DataCite DOI
	src, err := api.getJSON("/dois/10.22002/D1.868")
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
	...
	if len(src) == 0 {
		t.Errorf("expected a response body from /dois/10.22002/D1.868")
	}
	if api.RateLimitLimit == 0 {
		t.Errorf("expected greater than zero, got %d", api.RateLimitLimit)
	}
...
func TestDoisLookup(t *testing.T) {
	api, err := NewDataCiteClient("datacite_test.go", MailTo)
	if err != nil {
		t.Error(err)
		t.FailNow()
	...
	for _, id := range ids {
		_, err = api.DoisJSON(id)
		if err != nil {
			t.Errorf("Run api.DoisJSON(%q), got unexpected error, %s", id, err)
		}
	}
}
